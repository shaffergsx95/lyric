        ...
        DOMAIN: vol.Schema(
            {
                ...
                vol.Optional(CONF_AWAY_PERIODS): vol.All(cv.ensure_list, cv.string),
            }
        )
    },
...
def setup_lyric(hass, lyric, config, url=None):
    ...
    # lyric.token is returning None
    # so the best test is to check lyric._token instead
...
def setup(hass, config):
    ...
    redirect_uri = conf.get(
        CONF_REDIRECT_URI, hass.config.api.base_url + "/api/lyric/authenticate"
    )
    ...
    def get(self, request):
        ...
        if "code" not in data or "state" not in data:
            return self.json_message(
                "Authentication failed, not the right " "variables, try again.",
                HTTP_BAD_REQUEST,
            )
        ...
        return self.json_message(
            "OK! All good. Got the response! You can close "
